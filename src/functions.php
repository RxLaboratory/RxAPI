<?php
    ...
    // === GITHUB ===
    function getUserRepo( $toolName, $user ) {
        global $db, $appsTable;
        ...
        $rep->bindValue(':name', $toolName, PDO::PARAM_STR);
        $ok = sqlRequest( $rep, "Successful request." );
        $result = array();
        if ($ok)
        {
            if ($v = $rep->fetch()) {
                if ($v['ghUser']) $result[] = $v['ghUser'];
                else $result[] = $user;
                if ($v['ghRepo']) $result[] = $v['ghRepo'];
                else $result[] = $toolName;
            }
            $rep->closeCursor();
        }
        return $result;
    ...
    function ghRelease($ghToolUser, $ghRepo, $prerelease=false) {
        ...
        $info = getUserRepo( $ghRepo, $ghToolUser );
        if (count($info) == 2) {
            $ghToolUser = $info[0];
            $ghRepo = $info[1];
        }
    ...
    function ghContributors($ghToolUser, $ghRepo) {
        global $db;
        ...
        // We can get the contributor count from the header sent by github
        global $ghUsername;
        global $ghToken;

        $userAgent = 'RxAPI/2.0 (PHP)';

        $url = "https://api.github.com/repos/{$ghToolUser}/{$ghRepo}/contributors?per_page=1&anon=true";
        $ch = curl_init( $url );
        curl_setopt( $ch, CURLOPT_USERAGENT, $userAgent );
        curl_setopt($ch, CURLOPT_USERPWD, "{$ghUsername}:{$ghToken}");
        curl_setopt($ch, CURLOPT_URL, $url);
        ...
        foreach($headers as $header) {
            if (stripos($header, 'link:') !== false) {
                $headerValue = trim( explode(':', $header, 2)[1] );
                $headerValues = explode( ',', $headerValue);
                foreach( $headerValues as $headerValue ) {
                    list($link, $rel) = explode( ';', $headerValue, 2);
                    if (trim($rel) == 'rel="last"') {
                        $link = str_replace(">", "", $link);
                        return (int)explode("&page=", $link, 2)[1];
                    }
                }
            }
        }
    ...
    function ghIssuesCount($ghToolUser, $ghRepo) {
        global $db;
        ...
        $gh = ghRequest( "https://api.github.com/repos/{$ghToolUser}/{$ghRepo}" );

        $gh = json_decode($gh, true);
        if(!isset($gh['open_issues_count'])) return -1;
        return $gh['open_issues_count'];
    }
    // === PATREON ===
    ...
    function stripeIncome($skipCache = false)
    {
        ...
        $has_more = true;
        $prevId = "";
        // today minus 30.5 days
        $monthStart = time() - 2635200;
        while($has_more)
        {
